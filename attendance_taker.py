...
class Face_Recognizer:
    ...
    def draw_note(self, img_rd):
        ...
        # Get the text size and calculate position
        text_size = cv2.getTextSize(text, self.font, font_scale, thickness)[0]
        text_x = (img_rd.shape[1] - text_size[0]) // 2
        text_y = 30

        # Draw black rectangle as background and put the white text
        cv2.rectangle(img_rd, (text_x - 10, text_y - text_size[1] - 10),
                      (text_x + text_size[0] + 10, text_y + 10), (0, 0, 0), -1)
        cv2.putText(img_rd, text, (text_x, text_y), self.font, font_scale, (255, 255, 255), thickness, cv2.LINE_AA)

        # ======================


        cv2.rectangle(img_rd, (5, 425), (230, 460), (0, 0, 0), -1)  # Black background
    ...
    def process(self, stream):
        ...
        if self.get_face_database():
            while stream.isOpened():
                ...
                # If cnt of faces changes, 0->1 or 1->0 or ...
                else:
                    ...
                    # Face cnt decreases: 1->0, 2->1, ...
                    if self.current_frame_face_cnt == 0:
                        logging.debug("  / No faces in this frame!!!")
                        # clear list of names and features
                        self.current_frame_face_name_list = []
                    # Face cnt increase: 0->1, 0->2, ..., 1->2, ...
                    else:
