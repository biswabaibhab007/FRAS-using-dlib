...
class Face_Register:
    def __init__(self):
        ...
        # self.font_step_title = cv2.FONT_ITALIC
        self.font_title = tkFont.Font(family='Helvetica', size=25, weight='bold')
        self.font_step_title = tkFont.Font(family='Helvetica', size=15)
    ...
    def GUI_info(self):
        ...
        tk.Label(self.frame_right_info,
                 ...
                 text="Delete all the faces from database").grid(row=5, column=0, columnspan=2, sticky=tk.W, padx=5, pady=(30, 3))
        tk.Button(self.frame_right_info,
                  text='Delete',
                  pady=2,
                  bg='#D70000',  # Red color
                  fg='white',  # White text color
                  command=self.GUI_clear_data).grid(row=6, column=0, columnspan=2, sticky=tk.W + tk.E, padx=10, pady=5)

        # Step 2: Input name and create folders for face
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="To enroll a new person").grid(row=7, column=0, sticky=tk.W, padx=5, pady=(30, 3))

        tk.Label(self.frame_right_info, text="Enter name here: ").grid(row=8, column=0, sticky=tk.W, padx=(10,0), pady=0)
        ...
        tk.Button(self.frame_right_info,
                  ...
                  bg='#28A745',
                  fg='white',
                  command=self.GUI_get_input_name).grid(row=8, column=2, padx=0)

        # Step 3: Save current face in frame
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="Now take multiple pictures of your face").grid(row=9, column=0, columnspan=2, sticky=tk.W, padx=5, pady=(30, 3))

        # ==============================
        tk.Button(self.frame_right_info,
                  text='Capture current face',
                  command=self.save_current_face,
                  pady=5,
                  bg='#0078D7',
                  fg='white',
                  font=('Arial', 10, 'bold')).grid(row=10, column=0, columnspan=2, sticky=tk.W + tk.E, padx=10, pady=5)
    ...
    def update_fps(self):
        ...
        self.label_fps_info["text"] = str(self.fps.__round__(0))
        ...
        if self.face_folder_created_flag:
            if self.current_frame_faces_cnt == 1:
                if not self.out_of_range_flag:
                    ...
                    self.log_all["text"] = "Please do not move out of range!"
            else:
            ...
            if len(faces) != 0:
                ...
                for k, d in enumerate(faces):
                    ...
                    if (d.right() + self.ww) > 640 or (d.bottom() + self.hh > 480) or (d.left() - self.ww < 0) or (
                            d.top() - self.hh < 0):
                        self.label_warning["text"] = "Please step back slightly from the camera"
